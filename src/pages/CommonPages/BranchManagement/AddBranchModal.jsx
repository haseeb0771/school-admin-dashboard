...
function AddBranchModal({ isOpen, onClose, fetchBranches }) {
  // Basic branch info state
  ...
  const handleBranchInfoChange = (e) => {
    const { name, value } = e.target;
    setBranchInfo((prev) => ({ ...prev, [name]: value }));
  };
  ...
  const addSection = (classIndex) => {
    ...
    const updatedClasses = [...branchInfo.classes];
    updatedClasses[classIndex].sections = [
      ...updatedClasses[classIndex].sections,
      { ...newSection, sectionName: newSection.sectionName.trim() },
    ];
    setBranchInfo((prev) => ({ ...prev, classes: updatedClasses }));
      ...
      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="mt-6">
            ...
            {/* Existing Classes */}
            {branchInfo.classes.map((classItem, classIndex) => (
