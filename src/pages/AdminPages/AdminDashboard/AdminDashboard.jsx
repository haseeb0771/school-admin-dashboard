  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const response = await fetch("http://localhost:3300/stats/branch", {
        ...
        data.branchStats.forEach((branch) => {
          totalStudents += branch.totalStudents || 0;
          activeStudents += branch.activeStudents || 0;
          passedOutStudents += branch.passedOutStudents || 0;
        });

        // Prepare branch data for dropdown and chart
        const branchChartData = data.branchStats.map((branch) => ({
